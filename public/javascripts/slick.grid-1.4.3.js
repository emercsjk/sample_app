 ...
 *
 * SlickGrid v1.4.3
 * (handleHover modifiziert, mouseOverEdit als Column-Option hinzugefügt)
 * 
 *
 * TODO:
 * - frozen columns
...
(function($) {
    ...
    function SlickGrid(container,data,columns,options) {
        ...
        // Für Selenium-Tests random uid durch festen Wert ersetzt, für uns
        // sehr sicher ohne böse Nebenwirkungen
        var uid = "slickgrid_" + 4223422342;
        ...
        //UID rausrücken, um Spaltenköpfe extern bearbeiten zu können
        this.uid = returnUid;
        ...
        function createColumnHeaders() {
            ...
            for (i = 0; i < columns.length; i++) {
                ...
                header.append("<span class='slick-column-name'>" + m.name + hfilter + "</span>")
                ...
                //Filter-Spans verstecken
                if (m.filter)
                	$(".hf" + m.id).hide();

                // Diese Funktion wird auch zum Neuzeichnen der ColumnHeader verwendet
                // Deswegen hier: Check, ob ein Filter aktiv ist? Dann entsprechend anzeigen
                for (fi=0; fi<filter.length; fi++){
                	if ((filter[fi].field == m.id) && filter[fi].active){
                		
                		//Filter-Icon anzeigen
                		$(".hf" + m.id).show();
                		//Spaltenkopf per CSS einfärben
                		$("#" + uid + m.id).addClass('cell-filter');
                	}
        ...
        function setSortColumn(columnId, ascending) {
            ...
            $headers.find(".slick-sort-indicator").removeClass("slick-sort-indicator-asc slick-sort-indicator-desc")
            	.addClass("slick-sort-indicator-no");
        ...
        function appendRowHtml(stringArray,row) {
            ...
            for (var i=0, cols=columns.length; i<cols; i++) {
                ...
                // TODO: Hier eventuell Grade-CSS einklinken? 
                
                if (highlightedCells && highlightedCells[row] && highlightedCells[row][m.id])
                    cellCss += (" " + options.cellHighlightCssClass);

                
                //Teste Inhalt auf die zugehörigen Grade-Regeln
                _g = grade[m.id];
                
                //alert (grade[m.id]);
                if (_g){
                    for (gi = 0; gi < _g.length; gi++){
                    	_gib = _g[gi].bool;
                    	_geval = "true";
                    	for (gib = 0; gib < _gib.length; gib++){
                    		_geval += " && " + d[m.id] + " " + _gib[gib];
            ...
            if (options.enableCellNavigation && !columns[cell].unselectable) {
                ...
                if (validated === true || (validated === null && options.editorLock.commitCurrentEdit())) {
                    ...
                    if(c.mouseOverEdit){
                    	...
                    	setSelectedCellAndRow($cell[0], (row === defaultGetLength()));
                }
            }
